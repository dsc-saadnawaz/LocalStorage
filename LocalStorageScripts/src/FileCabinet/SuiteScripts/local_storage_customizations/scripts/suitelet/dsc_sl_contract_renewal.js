 ...
 */
define(['N/record', 'N/ui/serverWidget', '../../lib/dsc_lib_utils.js', '../../lib/dsc_lib_constants.js', 'N/search', 'N/ui/message'],
    (record, serverWidget, utilsLib, constantsLib, search, message) => {
        const onRequest = context => {
            ...
            try {
                ...
                if (request.method == "GET") {
                    ...
                    let parentContractId = request.parameters.custpage_parent_contract_id;
                    let contractLineDuration = request.parameters.custpage_contract_line_duration;
                    let contractStartDate = request.parameters.custpage_contract_start_date;
                    let existingContractLineID = request.parameters.custpage_parent_existing_contract_line_id;
                    let contractLinePaymentMode = request.parameters.custpage_contract_line_payment_mode;
                    let storageUnitRate = 10 //request.parameters.custpage_contract_line_payment_mode;
        ...
        const createForm = (parentContractRecordId, existingContractLineId, messageObj, contractEndDate) => {
            ...
            try {
                ...
                if (contractEndDate) {
                    ...
                    if (getDate) {
                        var calculateStartDate = new Date(getDate.setDate(getDate.getDate() + 1));
                    }
                }
                        ...
                        for (let i = 0; i < getLineCount; i++) {
                            ...
                            if (itemId != constantsLib.LOCAL_STORAGE_UNIT_ITEM_ID) {
                                ...
                                newSalesOrder.setSublistValue({
                                    ...
                                    fieldId: "rate",
                                    line: i,
                                    value: parseFloat(storageUnitRate).toFixed(2)
                                })
                            }
