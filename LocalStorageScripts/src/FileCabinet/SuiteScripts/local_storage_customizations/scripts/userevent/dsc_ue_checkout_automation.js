 ...
 */
define(['N/record', 'N/search', '../../lib/dsc_lib_utils.js', '../../lib/dsc_lib_constants.js'],
    (record, search, utilsLib, constantsLib) => {
        const afterSubmit = context => {
            ...
            try {
                ...
                if (context.type == context.UserEventType.CREATE) {
                    ...
                    let checkoutId = recObj.id;
                    let agreementId = recObj.getValue('custrecord_dsc_cf_agreement_no');
                    if (agreementId && checkoutId) {
                        const updatedParentContractId = record.submitFields({
                            ...
                            values: {
                                ...
                                custrecord_dsc_cf_checkout: checkoutId
                            }
                        });
                    }
                }
                if (context.type == context.UserEventType.EDIT) {
                    ...
                    let checkoutStatusNew = recordObj.getValue('custrecord_dsc_cf_checkout_status');
                    let checkoutStatusold = recordObjOld.getValue('custrecord_dsc_cf_checkout_status');
                    let checkoutStorageUnit = recordObj.getValue('custrecord_dsc_cof_storage_unit');

                    if (recordId && (checkoutStatusold != checkoutStatusNew) && checkoutStatusNew == constantsLib.FIELD_VALUES.CHECKOUT_STATUS_CLOSED) {
